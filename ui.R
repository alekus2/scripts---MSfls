...
ui <- tagList(
  tags$head(
    tags$style(HTML(paste0("
      ...
      .navbar { background-color: ", bracell_primary, " !important; margin-bottom: 0; border: none; border-radius: 0; }
      ...
      .tab-content { padding: 20px; background-color: #f4f4f4; border-top: 2px solid ", bracell_secondary, "; }
    ...
    tabPanel("Dados", icon = icon("file-upload"),
      sidebarLayout(
        sidebarPanel(
          ...
          radioButtons("shape_input_pergunta_arudek", "Formato do shape de entrada?", choices = list("P_SDE_BRACELL_PUB.VW_GIS_POL_US" = 1, "Outro" = 0), selected = 1),
          ...
          radioButtons("recomendacao_pergunta_upload", "Deseja realizar o upload do arquivo de recomendação", choices = list("Sim" = 1, "Não" = 0), selected = 1),
          ...
          radioButtons("parcelas_existentes_lancar", "Deseja informar as parcelas já existentes?", choices = list("Sim" = 1, "Não" = 0), selected = 0),
        ...
        tabPanel("DOWNLOAD",
          fluidPage(
            ...
            wellPanel(
              ...
              br(), br(),
              div(style = "color:red;font-weight:bold;", "O nome do arquivo será gerado automaticamente com data e hora.")
            )
          )
        )
      )
  ...
  tags$script(HTML("
    Shiny.addCustomMessageHandler('update_progress',function(percent){$('#progress-bar').css('width',percent+'%');$('#progress-bar').text(percent+'%');});
    ...
    $(document).on('click','.navbar-nav > li',function(){$('.navbar-nav > li').removeClass('active');$(this).addClass('active');});
  "))
)
