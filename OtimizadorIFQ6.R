...
validacao_ifq6 <- function(paths) {
  ...
  ler_ifq6 <- function(path) {
    df <- tryCatch(read_excel(path, sheet = 0), error = function(e) NULL)
...
[33] "DESCRIÇÃO"          "FECHAMENTO"        
Selecione (1=L,2=B,3=P): 1
Error in `bind_rows()` at Automação em R/OtimizadorIFQ6/OtimizadorIFQ6.R:62:3:
! Can't combine `..1$CD_TALHAO` <character> and `..10$CD_TALHAO` <double>.
