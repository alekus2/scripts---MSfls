  ...
  public = list(
    validacao = function(paths) {
      ...
      for (p in paths) {
        ...
        nome_arquivo <- toupper(basename(p))
        if (str_detect(nome_arquivo, "LEBATEC")) {
          base <- "lebatec"
        } else if (str_detect(nome_arquivo, "BRAVORE")) {
          base <- "bravore"
        } else if (str_detect(nome_arquivo, "PROPRIA")) {
          base <- "propria"
        } else {
          escolha <- ""
        ...
        if (resp == "s") {
          ...
          repeat {
            out <- file.path(pasta_output,
                             glue("{nome_base}_{str_pad(cnt,2,'0')}.xlsx"))
      ...
      # calcula Ht_media e ordena
      df_final <- df_final %>%
        mutate(Ht_media = as.numeric(replace_na(NM_ALTURA, 0))) %>%
      ...
      repeat {
        out2 <- file.path(pasta_output,
                          glue("{nome_base2}_{str_pad(cnt2,2,'0')}.xlsx"))
...
# exemplo de uso
pasta_dados <- "F:/Qualidade_Florestal/.../dados at"
arquivos <- list.files(
  path       = pasta_dados,
  pattern    = "\\.xlsx$",
  full.names = TRUE
