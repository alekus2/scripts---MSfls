...
Erro: objeto 'calc_metrics' não encontrado
Além disso: Mensagen de aviso:
Values from `Ht_media` are not uniquely identified; output will contain list-cols.
* Use `values_fn = list` to suppress this warning.
* Use `values_fn = {summary_fun}` to summarise duplicates.
* Use the following dplyr code to identify duplicates.
  {data} |>
  dplyr::summarise(n = dplyr::n(), .by = c(chave_stand, CD_PROJETO, CD_TALHAO, NM_PARCELA, NM_AREA_PARCELA, NM_COVA_ORDENADO)) |>
  dplyr::filter(n > 1L)
