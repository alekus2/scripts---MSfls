...
Lembrando que a coluna Área (ha) tem esse pequeno espaço entre ele!!


Quantidade de 'VERIFICAR': 0
---------------------------------------------------------------------------
UnboundLocalError                         Traceback (most recent call last)
<ipython-input-18-a86ac3144d2f> in <cell line: 0>()
    189     "/content/Cadastro SGF (correto).xlsx"
    190 ]
--> 191 otimizador.validacao(arquivos)

<ipython-input-18-a86ac3144d2f> in validacao(self, paths)
    167 
    168         # grava tudo sempre
--> 169         with pd.ExcelWriter(out, engine="openpyxl") as w:
    170             df_cadastro.drop(columns=["Talhão_z3"], inplace=True)
    171             df_cadastro.to_excel(w, sheet_name="Cadastro_SGF", index=False)
