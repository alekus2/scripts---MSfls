...
class OtimizadorIFQ6:
    def validacao(self, paths):
        ...
        df_tabela["PV50"] = df_tabela["PV50"].map(lambda x: f"{x:.1f}%".replace(".",","))
        ...
        df_D_resultados["PV50"] = df_D_resultados["PV50"].map(lambda x: f"{x:.1f}%".replace(".",","))
        counts = (
            df_final
            .groupby(["CD_PROJETO","CD_TALHAO","NM_PARCELA"])["CD_01"]
            .value_counts()
...
#media ht tem que existir em df_tabela e df_D_tabela_resultados mas a coluna deve ficar entre pits e sobrevivencia
