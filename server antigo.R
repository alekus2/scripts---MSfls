...
server <- function(input, output, session) {
  ...
  observeEvent(input$confirmar, {
    ...
    output$recomend_text <- renderText({
      req(input$recomend)
      paste("Upload realizado referente a recomendaÃ§Ã£o de parcelas:", input$recomend$name)
    })
    
    output$parc_exist_text <- renderText({
      if (input$parcelas_existentes_lancar == 1) {
        req(input$parc_exist)
        paste("Upload realizado referente as parcelas jÃ¡ existentes:", input$parc_exist$name)
      } else {
        paste0("Upload de parcelas existentes nÃ£o realizado.")
      }
    })
    
    output$confirmation <- renderText({
      req(input$forma_parcela, input$tipo_parcela, input$distancia_minima)
      paste("Forma Parcela:", input$forma_parcela, "Tipo Parcela:", input$tipo_parcela, "DistÃ¢ncia MÃ­nima:", input$distancia_minima)
    })
  })
