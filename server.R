...
server <- function(input, output, session) {
  ...
  observeEvent(input$db_connect, {
    ...
    talhao_tables <- grep("talhao", tbls, ignore.case = TRUE, value = TRUE)
  ...
  shape <- reactive({
    if (input$data_source == "upload") {
      ...
      if (isTRUE(session$userData$db_os_auth)) {
        dsn <- paste0("/@", input$db_service)
      } else {
  ...
  # 7) gerar_parcelas + sobrevivência
  ...
  output$plot <- renderPlot({
    ...
    ggplot()+
      ...
      ggtitle(paste0("Número de parcelas recomendadas: ",num_rec," (Área: ",round(area_ha,2)," ha)"))+
